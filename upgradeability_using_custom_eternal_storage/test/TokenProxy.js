...
contract('TokenProxy with custom Eternal Storage', ([_, proxyOwner, tokenOwner, anotherAccount]) => {
  ...
  describe('upgrade and call', function () {
    ...
    describe('when the new implementation is not the zero address', function () {
      ...
      describe('when the sender is the proxy owner', function () {
        ...
        it('upgrades to the given version', async function () {
          await proxy.upgradeToAndCall('0', impl_v0.address, initializeData, { from })
      ...
      describe('when the sender is not the proxy owner', function () {
        ...
        it('reverts', async function () {
          await assertRevert(proxy.upgradeToAndCall('0', impl_v0.address, initializeData, { from }))
        })
      })
    })
  ...
  describe('delegatecall', function () {
    ...
    describe('when an initial implementation was given', function () {
      ...
      beforeEach(async () => await proxy.upgradeToAndCall('0', impl_v0.address, initializeData, { from: proxyOwner }))
      ...
      describe('when there was another upgrade', function () {
        beforeEach(async () => {
          await token_v0.mint(sender, 100, { from: tokenOwner })
          await proxy.upgradeTo('1', impl_v1.address, { from: proxyOwner })
        })
